...
@app.route('/api/settings/trmnl-webhooks', methods=['POST'])
@jwt_required
def update_trmnl_webhooks():
    """Update TRMNL webhooks for the current user."""
    try:
        data = request.get_json()
        ...
        # Send to auth service
        response = requests.post(
            f'{AUTH_URL}/api/auth/settings/trmnl-webhooks',
            headers={'Authorization': f'Bearer {token}'},
        ...
        if response.status_code == 200:
            return jsonify({
                'success': True,
                'message': 'TRMNL webhooks updated successfully'
            })
        else:
            error_data = response.json() if response.content else {}
            return jsonify({
                'success': False,
                'error': error_data.get('error', 'Failed to update TRMNL webhooks')
            }), response.status_code

    except Exception as e:
        logger.error(f"Error updating TRMNL webhooks: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

def get_trmnl_todos_payload(notes_manager):
    """Generate payload for todos webhook type."""
...
def get_trmnl_stats_payload(notes_manager):
    """Generate payload for stats webhook type."""
    from datetime import date

    # Get all notes (use a high limit to get everything)
...
@jwt_required
def push_to_trmnl():
    """Push data to all enabled TRMNL webhooks."""
    try:
        token = session.get('access_token')
        ...
        if response.status_code != 200:
            return jsonify({
                'success': False,
                'error': 'Failed to fetch TRMNL webhooks'
            }), 500
        ...
        if not enabled_webhooks:
            return jsonify({
                'success': False,
                'error': 'No TRMNL webhooks configured'
            }), 400

        notes_manager = get_notes_manager()

        # Cache payloads by type to avoid regenerating
        payloads = {}
        ...
        for webhook in enabled_webhooks:
            ...
            if webhook_type not in payloads:
                if webhook_type == 'stats':
                    payloads[webhook_type] = get_trmnl_stats_payload(notes_manager)
                else:
                    payloads[webhook_type] = get_trmnl_todos_payload(notes_manager)

            payload = payloads[webhook_type]
        ...
        return jsonify({
            ...
            'message': f'Pushed to {successful}/{len(results)} webhooks',
            'results': results
        })
...
def execute_command():
    ...
    try:
        ...
        if cmd == "list":
            ...
            if not content:
                ...
                if cmd == "project":
                    ...
                    if not todo_notes:
                        ...
                        # Standard todo list with filtering (same logic as todos command)
                        ...
                        for note in todo_notes:
                            ...
                            if is_done and not show_all and not (is_done_today and donetoday):
                                continue

                            # Skip canceled todos if not explicitly requested
                            if is_canceled and not cancel_flag:
                                continue
                    ...
                    # Get all notes tagged with 'idea'
                    idea_notes = notes_manager.get_notes_by_tag('idea')
                    ...
                    if not idea_notes:
                        ...
                        if filter_tags:
                            ...
                            if not idea_notes:
                                filter_str = "', '".join(filter_tags)
                                result = ZettlFormatter.warning(f"No ideas found with all tags: '{filter_str}'.")
                                return jsonify({'result': process_for_web(result)})
                        ...
                        for note in idea_notes:
                            ...
                            if not categories:
                                ...
                                if is_canceled:
                                    if combined_category not in canceled_by_category:
                                        canceled_by_category[combined_category] = []
                                    if note not in canceled_by_category[combined_category]:
                                        canceled_by_category[combined_category].append(note)
                                elif is_done:
                                    if combined_category not in done_by_category:
                                        done_by_category[combined_category] = []
                                    if note not in done_by_category[combined_category]:
                                        done_by_category[combined_category].append(note)
                                else:
                                    if combined_category not in active_by_category:
                                        active_by_category[combined_category] = []
                                    if note not in active_by_category[combined_category]:
                                        active_by_category[combined_category].append(note)
                        ...
                        # Check if there are any ideas to display
                        if (not active_by_category and not uncategorized_active and
                            (not show_all or (not done_by_category and not uncategorized_done)) and
                            (not cancel_flag or (not canceled_by_category and not uncategorized_canceled))):
                            result = ZettlFormatter.warning("No ideas match your criteria.")
                            return jsonify({'result': process_for_web(result)})

                        # Helper function
                        def display_group(category_dict, uncategorized_list, header_text):
                            output = ""
                            if header_text:
                                output += f"{header_text}\n\n"

                            if category_dict:
                                for category, notes_list in sorted(category_dict.items()):
                                    if " - " in category:
                                        tags_in_cat = category.split(" - ")
                                        formatted_tags = [ZettlFormatter.tag(t) for t in tags_in_cat]
                                        category_display = " - ".join(formatted_tags)
                                        output += f"{category_display} ({len(notes_list)})\n\n"
                                    else:
                        ...
                        # Display results
                        result = ""
                        if active_by_category or uncategorized_active:
                            active_header = ZettlFormatter.header(f"Active {' '.join(header_parts)} ({len(unique_active_ids)} total)")
                            result += display_group(active_by_category, uncategorized_active, active_header)

                        if show_all and (done_by_category or uncategorized_done):
                            done_header = ZettlFormatter.header(f"Completed {' '.join(header_parts)} ({len(unique_done_ids)} total)")
                            result += "\n" + display_group(done_by_category, uncategorized_done, done_header)

                        if cancel_flag and (canceled_by_category or uncategorized_canceled):
                            canceled_header = ZettlFormatter.header(f"Canceled {' '.join(header_parts)} ({len(unique_canceled_ids)} total)")
                            result += "\n" + display_group(canceled_by_category, uncategorized_canceled, canceled_header)

                elif cmd == "note":
                    # NOTE LIST MODE - Full CLI behavior with filtering
                    # Extract options
                    show_all = 'all' in options or 'a' in flags
                    cancel_flag = 'cancel' in options or 'c' in flags
                    show_full = 'full' in options or 'f' in flags
                    ...
                    # Get all notes tagged with 'note'
                    note_notes = notes_manager.get_notes_by_tag('note')
                    ...
                    if not note_notes:
                        ...
                        if filter_tags:
                            ...
                            if not note_notes:
                                filter_str = "', '".join(filter_tags)
                                result = ZettlFormatter.warning(f"No notes found with all tags: '{filter_str}'.")
                                return jsonify({'result': process_for_web(result)})
                        ...
                        if (not active_by_category and not uncategorized_active and
                            (not show_all or (not done_by_category and not uncategorized_done)) and
                            (not cancel_flag or (not canceled_by_category and not uncategorized_canceled))):
                            result = ZettlFormatter.warning("No notes match your criteria.")
                            return jsonify({'result': process_for_web(result)})
                ...
                if cmd == "project" and link_ids:
                    if len(link_ids) > 1:
                        ...
                        try:
                            ...
                            # Verify it's a project
                            if 'project' not in [t.lower() for t in project_tags]:
                                result = ZettlFormatter.error(f"Note '{project_id}' is not a project.\n")
                            else:
                            ...
                            result = ZettlFormatter.error(f"Project '{project_id}' not found.\n")
            ...
            # Handle various ways the tag command is used
            # Usage: tag                       - List all tags
            #        tag note_id               - Show tags for note
            #        tag note_id "tag1"        - Add single tag
            #        tag note_id "tag1 tag2..." - Add multiple tags
            #        tag note_id "tag1" -r     - Remove tag(s)
            ...
            if not remaining_args:
                ...
                # 1. CLI-style with -t option: tag note_id1 note_id2 ... -t "tag1 tag2"
                # 2. Single note with positional args: tag note_id tag1 tag2 tag3
                # 3. Single note with quoted string: tag note_id "tag1 tag2 tag3"

                # First check for -t option (CLI-style) - supports multiple note IDs
                tag_option = options.get('tag', options.get('t', ''))
                if tag_option:
                    # All remaining_args are note IDs when using -t option
                    note_ids = remaining_args
                    tag_list = tag_option.split() if ' ' in tag_option else [tag_option]
                else:
                    # Legacy format: first arg is note_id, rest are tags
                    note_ids = [remaining_args[0]]
                ...
                if tag_list:
                    ...
                    for note_id in note_ids:
                        try:
                            if remove_mode:
                                for t in tag_list:
                                    notes_manager.delete_tag(note_id, t)
                            else:
                                if len(tag_list) == 1:
                    ...
                    # Report results
                    action = "Removed" if remove_mode else "Added"
                    tag_str = ', '.join(tag_list)
                    if success_count > 0:
                        if len(note_ids) == 1:
                            result = ZettlFormatter.success(f"{action} tag(s) '{tag_str}' {'from' if remove_mode else 'to'} note #{note_ids[0]}") + "\n"
                        else:
                            result = ZettlFormatter.success(f"{action} tag(s) '{tag_str}' {'from' if remove_mode else 'to'} {success_count} note(s)") + "\n"
                    else:
                        result = ""

                    if failed:
                        for f in failed:
                            result += ZettlFormatter.error(f"Failed for note #{f['id']}: {f['error']}") + "\n"
                else:
                    result = ""
                    # Show all tags for the note (only for single note without tags)
                    if len(note_ids) == 1:
                        tags = notes_manager.get_tags(note_ids[0])
                        if tags:
                            result = f"Tags for note #{note_ids[0]}: {', '.join([ZettlFormatter.tag(t) for t in tags])}"
                        else:
                            result = f"No tags for note #{note_ids[0]}"
            ...
            # Push data to TRMNL device via API endpoint
            try:
                token = session.get('access_token')

                # Call our own push endpoint
                response = requests.post(
                    f'http://localhost:5000/api/settings/trmnl-push',
                    headers={'Authorization': f'Bearer {token}'},
                ...
                if response.status_code == 200:
                    data = response.json()
                    if data.get('success'):
                        result = ZettlFormatter.success(data.get('message', 'Pushed to TRMNL'))
                    else:
                        result = ZettlFormatter.error(data.get('error', 'Failed to push to TRMNL'))
                else:
                    result = ZettlFormatter.error("Failed to push to TRMNL")
